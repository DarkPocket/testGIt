...
//const int
#define samplingCount  200  //抽样点数目
const double pi = 3.1415926;
const int totalAmplitude = 300; //每个抽样点对应时间片
const double delta = 2.0 /  200;//  samplingCount; //抽样弧度的增量
...
{
        DWORD startTime = 0;int j = 0;
        for (  j = 0; ; j = (j + 1) % samplingCount)
        {
...
void CpuSin(int cpuIndex)
{
        ...
        double radian = 0;
        int amplitude = totalAmplitude / 2;int i = 0;
        ...
        if (cpuIndex < 0)
        {
                HANDLE* threads = new HANDLE[num_processors];int i = 0;
